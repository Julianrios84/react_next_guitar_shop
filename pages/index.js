...
export default function Home({ guitars, posts, course }) {
  return (
    <>
      <Layout title={'Home'} description={'Music blog, guitar sales and more'}>
...
export async function getStaticProps() {
  const urlGuitars = `${process.env.API_URL}/guitars?populate=picture`;
  const urlPosts = `${process.env.API_URL}/posts?populate=image`;
  const urlCourse = `${process.env.API_URL}/course?populate=picture`;
